...
$data = [
    ...
    'base_rate' => (string)($type['base_rate'] ?? '0.00'),
    'max_adults' => (int)($type['max_adults'] ?? 2),
    'max_children' => (int)($type['max_children'] ?? 0),
    'image_path' => (string)($type['image_path'] ?? ''),
];
...
if (Request::isPost()) {
    ...
    $ok = false;
    if (!$errors) {
        $ok = $typeService->update($id, $data, $errors);
    }
    if ($ok) {
        Flash::set('success', 'Room type updated successfully.');
        Response::redirect('types_view.php?id=' . $id);
    }
}

$pageTitle = 'Edit Room Type - Hotel Management System';
$pendingApprovals = [];
...
<section id="content">
    ...
    <main class="w-full px-6 py-6">
        <div class="mb-6">
            <h1 class="text-2xl font-light text-gray-900">Edit Room Type</h1>
            <p class="text-sm text-gray-500 mt-1">Update room type</p>
        </div>
